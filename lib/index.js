...
function slim(item) {
  ...
  const pkgName = check && check.status === "pass" && typeof check.pkgName === "string" ? check.pkgName : null;
  ...
  return {
    ...
    pkg: pkgName,
    // `--profile web` is required by the dsh CLI; verified installable form.
    cmd: pkgName ? `dsh plugin --profile web add ${pkgName}` : null,
  };
}
